...
defmodule Codeflow.Runtime do
  ...
  def walk(node, pid, parent, application, kind, modules, depth, state, limit, max_depth) do
    ...
    cond do
      ...
      true ->
        ...
        info = if is_list(info), do: info, else: []
        ...
        entry = %{id: key, pid: key, parentId: parent, application: application, label: label,
          ...
          metrics: %{memory: Keyword.get(info, :memory), messageQueueLength: Keyword.get(info, :message_queue_len),
            reductions: Keyword.get(info, :reductions), status: to_string(Keyword.get(info, :status, :exited))}}
  ...
  def run do
    ...
    pids = case rpc(target, :erlang, :processes, []) do
    ...
    IO.puts("CODEFLOW_RUNTIME:" <> JSON.encode!(%{applications: applications, processes: processes, truncated: state.truncated}))
  end
end
