...
function source(path,content){
  const elixir=context.Parser.analyzeElixir(content,path);
...
test('declared Elixir namespaces and OTP roles enrich native blocks without dropping member files',()=>{
  ...
  const diagram=context.buildArchitectureDiagram(files);
  ...
  assert.match(context.generateMermaidBlockDiagram(diagram,true,false),/Example.SessionTest/);
  const data={files,connections:[],functions:[],fnStats:{},issues:[],suggestions:[],deadFunctions:[],stats:{},architectureDiagram:diagram};
  const enriched=context.buildBeamAnalysisData({data:data,snapshot:{status:'ready',nodes:[],edges:[{source:'lib/client.ex',target:'lib/unrelated_name_79.ex',kind:'compile'}]}});
...
test('Mix takes precedence over generated HTML and examples remain represented in the ordinary overview',()=>{
  ...
  for(const title of ['Examples','Benchmarks']){
    ...
    assert.ok(context.getVisibleArchitectureBlocks(diagram.blocks,false,false).includes(block),'examples stay available without toggling tests');
...
test('native Mermaid export retains every source relationship beyond the former 48-edge limit',()=>{
  ...
  const diagram=context.buildArchitectureDiagram([caller,...targets]);
