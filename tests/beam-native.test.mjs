...
import { buildTree } from '../src/project/tree.mjs';
import { buildBeamAnalysisData } from '../src/analysis/evidence.mjs';
import { calcBlast, calcHealth } from '../src/analysis/metrics.mjs';
import { getConnectedFilePaths } from '../src/investigation/navigation.mjs';
...
function nativeData(files = analyzedFiles()) {
  ...
  return { files, functions, connections: [], fnStats: {}, folders: ['lib', 'test'], tree: buildTree(files),
...
test('compiler edges use native provider-to-consumer direction for impact and Code navigation', () => {
  const data = buildBeamAnalysisData({ data: nativeData(), snapshot, excludePatterns: [] });
  ...
  const serviceImpact = calcBlast('lib/service.ex', data.connections, data.files);
  ...
  const callerImpact = calcBlast('lib/caller.ex', data.connections, data.files);
  assert.deepEqual(plain(callerImpact.dependencies).sort(), ['lib/data.ex', 'lib/macro.ex', 'lib/service.ex']);
  assert.deepEqual(plain(getConnectedFilePaths('lib/caller.ex', data.connections)).sort(),
...
test('compiler enrichment retains source analysis and isolated files', () => {
  ...
  const data = buildBeamAnalysisData({ data: nativeData(input), snapshot, excludePatterns: [] });
  ...
  assert.equal(typeof calcHealth(data).score, 'number');
...
test('excluded or unread sources cannot leave dangling compiler connections', () => {
  ...
  const data = buildBeamAnalysisData({ data: nativeData(analyzed), snapshot, excludePatterns: ['lib/service.ex'] });
  ...
  assert.deepEqual(plain(getConnectedFilePaths('lib/second.ex', data.connections)), []);
});

test('unavailable compiler evidence retains the native analysis', () => {
  const data = buildBeamAnalysisData({ data: nativeData(), snapshot: {
    status: 'unavailable', nodes: [], edges: [], warnings: ['No compiler artifacts'],
  }, excludePatterns: [] });
  assert.equal(data.files.length, 7);
  assert.deepEqual(plain(data.connections), []);
  assert.equal(calcHealth(data).score, 100);
});

const canvasFixture = () => buildBeamAnalysisData({ data: nativeData(), snapshot });
test('workspace restoration retains navigation and placement while pruning removed files', () => {
  const restored = restoreWorkspace({version:1,scope:'lib',selected:'lib/caller.ex',view:'code',
  ...
  assert.equal(restoreWorkspace({version:2},canvasFixture()),null);
});
